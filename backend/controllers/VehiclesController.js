...
const Search = async (req, res) => {
  ...
  try {
    // Normalize search (uppercase, remove spaces and hyphens)
    search = search.toUpperCase().replace(/[\s-]/g, '');

    // Fetch and normalize data on the fly
    const vehicles = await addvehicleModel.find();

    // Find first match
    const findData = vehicles.find(v => {
      const normalizedVehicleNumber = v.vehicleNumber.toUpperCase().replace(/[\s-]/g, '');
      const normalizedRcNumber = v.rcnumber.toUpperCase().replace(/[\s-]/g, '');
      return normalizedVehicleNumber.includes(search) || normalizedRcNumber.includes(search);
    });
